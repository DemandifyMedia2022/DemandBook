    ...
    <section className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        {catalogGroups.map((group, idx) => (
          <div
            key={idx}
            className="bg-surface rounded-xl shadow-sm border border-outline-variant flex flex-col overflow-hidden hover:shadow-md transition-shadow"
          >
            <div className="p-4 bg-surface-container-low border-b border-outline-variant flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h4 className="font-bold text-sm text-on-surface">{group.title}</h4>
              </div>
              ...
              <table className="w-full text-left">
                <tbody className="divide-y divide-outline-variant/50 bg-surface-container-lowest">
                  {group.reports.map((report, rIdx) => (
                    ...
                    >
                      <td className="px-4 py-3 text-xs font-semibold text-on-surface-variant flex items-center gap-1.5">
                        {report.name}
                        {report.isPro && (
